...
superficie <- superficie %>% 
  mutate(sup_prop = round(superficie_afectada_por_incendios_total_hectareas/km2,2)) #Creamos la variable sup_prop para normalizar la prop de Ha afectadas, redondeamos a 2 decimales
```
...
#Probmas el mapa con la cantidad de incendios
cantidad_mapa %>% 
  filter(anio == 2018) %>% 
  ggplot()+ #Graficamos cantidad de incendios x provincia en 2018
  geom_sf(data = cantidad_mapa) +
  geom_sf(aes(fill = incendio_total_numero))+
  coord_sf(xlim = c(-74, -52), ylim = c(-56, -20))+ # sacamos la antartida que deforma el mapa
  theme_void()+
