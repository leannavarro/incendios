...
Ahora vamos a sumar registros de medios locales y provinciales de Santa Fe y Córdoba que no se encontrbana en la API, para nutrir el análisis. Scrapeamos articulos sobre el mismo tema, para fechas similares.
...
while(contador < 5){ #traigo las 4 páginas que entran en el período establecido
  ...
  for(ele in 1:length(title_click)) {
    ...
    titulo <- driver$client$findElement(using = "css", value = ".news-header-title")
    title1 <- append(title1, value =titulo$getElementText()) #guardo los comentarios
    descripcion <- driver$client$findElement(using = "css", value = ".news-header-description")
...
while(contador < 3){ #traigo las 4 páginas que entran en el período establecido
  
  title_click <- driver$client$findElements(using = "css", value = ".result-title a") #busco los comentarios
  for(ele in 1:length(title_click)) {
    title_click[[ele]]$clickElement()
    ...
    title2 <- append(title2, value =titulo$getElementText()) #guardo los comentarios
...
data_medios <- filter(data_medios[-c(53, 134, 145, 190, 199,303,329,336,338,413,419,420,421,422,423,424,425), ])
