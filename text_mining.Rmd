...
stop_personalizadas <- tibble(word = c("incendios", "incendio", "córdoba", "dos", "tres", "mil", "san", "amp", 
                                       "si", "10", "así", "ldquo", "rdquo", "br"))
...
bigrams_united %>% group_by(name_medio) %>% 
  ...
  geom_col() +
  theme_classic() +
  facet_wrap(~name_medio,scales = "free", ncol = 3, drop = TRUE) + 
  coord_flip()
